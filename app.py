...
- Left sidebar (closed by default) with toggle ☰ (works via JS)
- Contact opens WhatsApp in new tab
- Try Dashboard scrolls to dashboard section
- Tabs removed; sections are anchors in-page
- Visible gold borders around main sections
- Progress, toast, back-to-top, responsive CSS
- All original data functions preserved
"""
...
# CSS and JS (we'll use placeholders and replace to avoid f-string/brace issues)
css_template = """
<style>
...
/* Section container with visible border */
.section-card {{
    margin: 18px 24px;
    padding: 18px;
    border-radius: 12px;
    border: 1px solid rgba(255,215,0,0.12);
    background: linear-gradient(180deg, rgba(0,36,77,0.02), rgba(0,0,0,0.02));
...
@media (max-width: 900px) {{
    .sidebar-custom {{ width: 220px; }}
    .hero {{ flex-direction: column; text-align:center; gap:12px; margin:14px 12px; }}
    .section-card {{ margin: 12px 12px; padding:12px; }}
}}
...
css = css_template.replace("{PRIMARY_DARK}", PRIMARY_DARK).replace("{GOLD}", GOLD).replace("{GOLD_LIGHT}", GOLD_LIGHT).replace("{CARD_BG}", CARD_BG)

st.markdown(css, unsafe_allow_html=True)

# Sidebar HTML (closed by default)
sidebar_html = f"""
<div id="sidebar-custom" class="sidebar-custom {'open' if st.session_state.sidebar_open else ''}">
    ...
    <div style="margin-top:12px; color:#e6eef8; font-size:13px;">
        ...
        <div style="margin-top:8px;"><a href="javascript:void(0)" onclick="openWhatsAppJS()" style="color:{GOLD}; text-decoration:none;">Contact Support</a></div>
    </div>
</div>
"""

# Render toggle button, sidebar, toast and back-to-top
st.markdown(f"""
    ...
    if file_list:
        ...
        for i, f in enumerate(file_list):
            st.markdown(
                ...
                f"<div>{i+1}. {f.name}</div><div style='opacity:0.9;'>{getattr(f, 'size', 0)//1024} KB</div></div>",
                unsafe_allow_html=True
            )
...
# ------------------ Page content: Sections (no Streamlit tabs) ------------------
    ...
    st.markdown(
        ...
        <div class="hero" role="banner">
            ...
            <div style="width:160px; text-align:center;">
                <img src="logo.png" width="120" onerror="this.style.display='none'"/>
...
with col2:
    # quick actions area (contact)
    st.markdown("<div style='text-align:right; margin-right:20px;'>", unsafe_allow_html=True)
    st.markdown(f"<button onclick=\"openWhatsAppJS()\" style='background:{GOLD}; color:black; border:none; padding:8px 12px; border-radius:8px; font-weight:700; cursor:pointer;'>📥 Contact (WhatsApp)</button>", unsafe_allow_html=True)
    st.markdown(f"<div style='text-align:right; color:#a9c1df; margin-top:10px;'>Last updated: <strong>{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</strong></div>", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    st.markdown("<p style='color:#ffd166;'>⚠️ No file uploaded yet for splitting.</p>", unsafe_allow_html=True)
...
if merge_files:
    ...
    st.markdown("<div style='color:#a9c1df;'>Upload files to merge multiple sheets into a consolidated Excel.</div>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)
...
if uploaded_images:
    ...
    st.markdown("Upload images to convert them into a single PDF.", unsafe_allow_html=True)
...
if dashboard_file:
    ...
    try:
        ...
        # (Detect periods & months — original logic preserved)
        ...
        for col in numeric_cols:
            ...
            if match:
                ...
                base_names.setdefault(base, []).append((col, period))
        valid_periods = {b:cols for b,cols in base_names.items() if len(cols) == 2}
        if valid_periods:
        ...
        if potential_months:
            ...
            df_long = df0.melt(id_vars=id_vars, value_vars=potential_months, var_name="Month", value_name="Value")
        ...
        # Filters: use Streamlit sidebar (keeps functionality)
        ...
        for ins in insights:
            ...
            <div style='background:linear-gradient(135deg,#003366,#001a33); border:1px solid {GOLD}; border-radius:12px; padding:15px; margin:10px 0; box-shadow:0 4px 10px rgba(0,0,0,0.4);'>
                <p style='color:{GOLD}; font-size:16px; font-weight:bold; margin-bottom:6px;'>{ins}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        if performance_group_col:
            try:
                ...
                if chart_type == "Bar":
                    fig = px.bar(group_total, x='Performance Group', y=kpi_measure_col, color='Performance Group', color_discrete_map=color_map, text=kpi_measure_col, title="Performance Groups")
        ...
        possible_dims = [c for c in final_df.columns if c != kpi_measure_col and c not in (date_cols if 'date_cols' in locals() else []) and c != rep_col]
        chosen_dim = None
    ...
    st.markdown("Upload an Excel/CSV file to generate an auto dashboard.", unsafe_allow_html=True)
...
st.markdown("<div class='section-card'>", unsafe_allow_html=True)
st.markdown(f"""
<div style='color:{GOLD}; font-weight:800; font-size:18px;'>🎯 Welcome to a free tool provided by the company admin.</div>
<br>
<h3 style='color:{GOLD};'>📌 How to Use</h3>
<ol style='color:white; font-size:15px; line-height:1.6;'>
    <li><strong>Upload Excel/CSV File (Splitter/Merge)</strong>: Use the Split & Merge section to split large sheets.</li>
...
# Finally: print CSS/JS string to ensure it's loaded (no-op)
